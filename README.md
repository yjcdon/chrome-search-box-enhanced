...
- 🚫 **禁用网站** — 在指定网站回退到 Chrome 原生查找或网站自带查找
...
点击扩展图标可打开禁用网站设置。默认会填入当前标签页域名，也可以手动输入其他域名后添加。

被禁用的网站不会拦截 `⌘F` / `Ctrl+F`，浏览器会继续使用 Chrome 原生查找或网站自带查找。禁用列表使用 `chrome.storage.local` 保存在本机。
...
```bash
pnpm dev          # 同时监听 content 和 popup 构建
pnpm dev:content  # 仅监听 content script 构建
pnpm dev:popup    # 仅监听 popup 构建
pnpm build        # 构建
```
